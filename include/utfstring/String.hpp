#pragma once
#include <utfstring/StringSlice.hpp>
...
namespace utfstring
{
    class String final
    {
        ...
        {
            std::char_traits<TChar>::copy(dest, src, size);
        }

        inline static void SetData(TChar* dest, TChar value, size_t size) noexcept
        {
            std::char_traits<TChar>::assign(dest, size, value);
        }
    ...
    public:
        ...
        {
            UTF_STRING_ASSERT(index < Size() && "Invalid index");
            return Data()[index];
        }
        ...
        [[nodiscard]] inline TCodepoint CodePointAt(size_t index) const
        {
            ...
            UTF_STRING_ASSERT(false && "Invalid index");
            return 0;
        }
        ...
        {
            UTF_STRING_ASSERT((count == 0 || str != nullptr) && "Couldn't append more than 0 chars from a null string");
            if (count == 0)
            {
                return *this;
            }
        ...
        template<class T>
        [[nodiscard]] inline ParseError TryParse(T& result)
        {
            return StringSlice(Data(), Size()).TryParse<T>(result);
        }
        ...
        [[nodiscard]] inline static String Join(const StringSlice& separator, const std::vector<StringSlice>& strings)
        {
            String result;
            size_t capacity = 0;
...
{
    template<>
    struct hash<utfstring::String>
    {
        inline size_t operator()(const utfstring::String& str) const noexcept
        {
